...
@interface DRPickerContainerView () <QCCardContentDelegate>

@property (nonatomic, assign) DRPickerShowPosition position;
@property (assign, nonatomic) CGFloat changeToHeight;
@property (weak, nonatomic) QCCardContainerController *cardContainerVc;
...
- (void)pickerViewHeightChange:(CGFloat)newHeight {
    if (self.position == QCCardContentPositionBottom) {
        self.cardContainerVc.minTopSpaceInSafeArea = kDRScreenHeight - newHeight;
...
- (void)setupCardContainerVc:(QCCardContainerController *)cardContainerVc {
    kDRWeakSelf
    self.cardContainerVc = cardContainerVc;
    ...
    if (self.position == DRPickerShowPositionBottom) {
        cardContainerVc.minTopSpaceInSafeArea = kDRScreenHeight - [self pickerViewHeight];
    }
