...
- (void)setOnLeftButtonTapBlock:(dispatch_block_t)onLeftButtonTapBlock {
    ...
    if (onLeftButtonTapBlock != nil) {
        [self.leftButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#2899FB"] forState:UIControlStateNormal];
    } else {
        [self.leftButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#3C3C43" alpha:0.85] forState:UIControlStateNormal];
    }
}
...
- (void)setOnTitleTapBlock:(dispatch_block_t)onTitleTapBlock {
    ...
    if (onTitleTapBlock != nil) {
        [self.titleButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#2899FB"] forState:UIControlStateNormal];
    } else {
...
- (instancetype)initWithContentType:(QCCardContentType)contentType position:(QCCardContentPosition)position {
    if (self = [super init]) {
        ...
        _showBottomBar = NO;
        _bottomBarTitle = @"取消";
        _bottomBarTintColor = [UIColor hx_colorWithHexRGBAString:@"#3C3C43" alpha:0.85];
...
- (UIButton *)leftButton {
    if (!_leftButton) {
        if (self.position == QCCardContentPositionBottom && self.contentType != QCCardContentTypeView) {
            ...
            [leftButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#3C3C43" alpha:0.85] forState:UIControlStateNormal];
...
- (UIButton *)rightButton {
    if (!_rightButton) {
        if (self.position == QCCardContentPositionBottom && self.contentType != QCCardContentTypeView) {
            ...
            [rightButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#2899FB"] forState:UIControlStateNormal];
            [rightButton setTitleColor:[UIColor hx_colorWithHexRGBAString:@"#3C3C43" alpha:0.3] forState:UIControlStateDisabled];
            [rightButton addTarget:self action:@selector(onConfirmAction) forControlEvents:UIControlEventTouchUpInside];
